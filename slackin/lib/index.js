...
export default function slackin ({
  ...
  if (blockDomains) {
    ...
    let blockDomainsHash = {}
    blockDomains.split(/[,\n]/).forEach(d => { if(!d.startsWith('#')) { blockDomainsHash[d] = true }})
    ...
    const captcha_callback = (err, resp) => {
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          if (blockDomains && blockDomains[hostname]) {
            // Direct block domain match, return success but do nothing.
            return res
            .status(200)
            .json({ msg: 'WOOT. Check your email!' })
          }

          dns.resolveMx(hostname, (err, mxAddresses) => {

            // Ignore errors as probably fine, otherwise a higher risk of false positives.
            if (blockDomains && mxAddresses && mxAddresses.some(mx => blockDomains[mx.exchange])) {
